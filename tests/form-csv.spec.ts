 ...
 *   CSV_PATH   — ruta al CSV     (default: tests/clientes-map.csv)
 */
import { test, expect } from '@playwright/test'
import { login } from './helpers/auth.js'
import { fillForm, fillFormFromMap, loginFromMap } from './helpers/fillForm.js'
import type { FieldMap, Cliente } from './helpers/types.js'
import fs from 'fs'
...
const fieldMap: FieldMap | null = hasFieldMap
  ? (JSON.parse(fs.readFileSync(fieldMapPath, 'utf8')) as FieldMap)
  : null
...
// ── Parser de CSV ─────────────────────────────────────────────────────────────
function parseCSV(filePath: string): Cliente[] {
